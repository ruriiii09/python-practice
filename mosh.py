...
if st.sidebar.button("スプシから取得"):
    try:
        ...
        st.sidebar.text(f"新規：{cell_new.value}名")
        st.sidebar.text(f"リピーター：{cell_repeat.value}名")
...
if st.button("📋 中間報告からデータを自動転記する"):
    sync_mid_report()

# text_area 等の key を session_state と合わせる
with col1:
    st.subheader("今日やったこと")
    done_items = st.multiselect("選択", ["・トイレ清掃","・作業場清掃","・冷蔵庫清掃","・墨壺清掃","・床水拭き","・床掃き","・amazon発注","・フレーバー発注",
                                       "・めいらく発注","・スイーツカット","・アルミカット","・季節サワー仕込み","・買い出し","・ゴミ捨て","・ドア、窓拭き","・マウピ、おしぼり補充","・フレーバー補充"], key="done")
    # text_area の値は key だけで管理するか、value に state を渡す
    done_add = st.text_area("追記（はじめに・をつけてね）", key="done_a")

with col2:
    st.subheader("明日やってほしいこと")
    next_items = st.multiselect("選択", ["・トイレ清掃","・作業場清掃","・冷蔵庫清掃","・墨壺清掃","・床水拭き","・床掃き","・amazon発注","・フレーバー発注",
                                       "・めいらく発注","・スイーツカット","・アルミカット","・季節サワー仕込み","・買い出し","・ゴミ捨て","・ドア、窓拭き","・マウピ、おしぼり補充","・フレーバー補充"], key="next")
    next_add = st.text_area("追記（はじめに・をつけてね）", key="next_a")

with col3:
    st.subheader("ないものや買ってほしいもの")
    buy_items = st.multiselect("選択", ["・ウタマロ","・ハイター"], key="buy")
    buy_add = st.text_area("追記（はじめに・をつけてね）", key="buy_a")

st.header("📝 共有事項")
names_input = st.text_area("来店された方のお名前（区切り自由）" , key="names_input")
...
"""
        report_data = {
        "done": done_items, "next": next_items, "buy": buy_items,
        "names": names_input, "cat1": cat1, "cat2": cat2, "cat3": cat3
        }
        save_report(str(datetime.now().date()), "中間", mid_report, report_data )
        st.info("中間報告を作成しました。お疲れ様でした！")
        st.code(mid_report)


with col_report2:
    if st.button("🚀 最終報告を生成"):
        # --- (ここにはさっきの長い最終報告フォーマットを入れる) ---
        final_report = f"""
====================================
**終業報告** 
====================================
...
"""
        ...
        save_report(str(datetime.now().date()), "終業", final_report, report_data)
        st.success("最終報告書が完成！お疲れ様でした。")
